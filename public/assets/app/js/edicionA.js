(function(window, document, $){
  ...
  app.init = function(){
    ...
    app.$btn_crear_anuncio = app.$form.find('#btn-editar-anuncio');

    $(document).on('click', '#btn-editar-anuncio', app.on_click_editar_anuncio );

    app.inicializar_elementos_de_formulario();
  };

  app.on_click_editar_anuncio = function(){
    var $btn = $(this);
  ...
  app.crear_anuncio = function( $btn ){
    ...
    setTimeout(function(){
      ajax({
        url: APP.url.inicio + '/ajax/editar-anuncio',
        type: 'POST',
        dataType: 'json',
        data: data,
        beforeSend: function(){
          ...
          if( response.html != 'ajax-error' && response.success === true ){
            $alert.html('<p>Anuncio Editado con éxito</p>');
            ...
            $alert.html('<p>No se pudo editar su anuncio, inténtalo de nuevo. Error: '+response.error+'</p>');
          }
